...
**第 0 步（最重要）：写设计概念。** 读完产品资料后，先用一句话写下"这本手册要让读者合上时感觉到什么"（例："像走进一所安静有实力的大学"）。之后每个选择——方向、构图、图片、取舍——都拿这句话来裁决。**只执行规则、没有概念的手册，就是"全都类似的风格"的根因。**

**然后选方向：风格由读者决定，不由产品决定。** 按 `style-directions.md` 的三问框架，先看 `audience`（给谁看）和手册目的（促成什么行动）——同一个产品，给企业决策者的版本和给青少年家长的版本风格必须不同；产品类别只做色板微调。拿不准用 editorial。

知识库的规则分三层（见 design-system.md §0）：原则要内化；默认值是高起点，**能说出理由就可以打破**；只有红线（重叠/变形/可读性/事实/合规）永远不能破。不要把默认规则当成限制发挥的天花板。

选定后推导四个色槽与字体配对，写入 HTML 顶部的 CSS 变量：
